...
- **实时调整**：滑动条即时调整 MCTS 模拟次数（50~800）、传统 AI 搜索深度（1~8）、并行对弈局数（1~10）
...
| **并行局数** | 滑动条 1~10，控制训练收集/演示模式下并发对弈局数 |

**演示模式自动循环**：AI vs AI 和 传统AI vs NN 模式下，每局结束后自动 2 秒后开始新局，无需手动点击。训练数据收集模式同样自动循环，持续不断收集数据并保存。
